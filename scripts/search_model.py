...
class EvolutionSearcher(object):
    ...
    def update_top_k(self, candidates, *, k, key, reverse=False):
        # 这个方法用于更新类属性`keep_top_k`中的一个列表，它保存了当前搜索中的前k个候选者（candidates）
        # - `candidates`：一个候选者列表，表示当前迭代中的候选者。
        # - `k`：一个整数，表示要保留的前k个候选者。
        # - `key`：一个函数，用于定义如何比较候选者的优劣。该函数将应用于候选者列表中的每个元素，并根据其返回值进行排序。
        # - `reverse`：一个布尔值，表示是否按降序排序（默认为False，即升序）。
    ...
    def sample_active_subnet(self):
        # 这个方法用于从可能的时间步中随机选择一些时间步，以生成一个激活的子网络（active subnet）
    ...
    # 将时间步均匀分割，对每个分割点赋予随机5-10的权重，生成分割点权重列表
    def sample_point_weight(self):
        ...
        # 插值
        # weights = np.interp(range(original_num_steps), split_points, weights_at_splitpoints)
    ...
    # 先不使用进化算法，在每次训练之前随机采样一些策略，然后训练，最后选择最优的策略
...
def cal_model_fid(model_path, sample_num):
    ...
    # with np.load("/home/zyx/zbb/tmp/samples_11-03-21.npz") as data:
    #     # 提取名为'arr'的数组
    #     arr_name=data.files
    #     arr = data[arr_name[0]]
    from evaluations.evaluator_v1 import cal_evalutions, FIDStatistics
